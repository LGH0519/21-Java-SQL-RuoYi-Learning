...
> **今日唯一目标：** 闭卷完成 SQL 六项测试，形成学习 ER 图、字段字典和 XML→SQL 对照。
>
> **今天不做：** 不看新视频，不通过就跳到框架阶段。 只完成本日主题；验收未通过就停在当天补练，不用“先往后做”掩盖缺口。

## 知识点速记（完成当天任务后用于复习）

- **SQL 链：**DDL→DML→DQL→事务→索引。
- **ER 图：**实体、主键、关联列、基数必须对应实际表。
- **字段字典：**字段、类型、可空、约束、说明是代码生成前输入。
- **XML：**MyBatis XML 把 Java Mapper 方法映射为 SQL。

### 本日必须形成的业务映射

前端查询条件最终会成为 XML 动态 SQL；ER/字典是 D33 正式设计前的练习。

### 本日重点、易混点与易面试点

图不是自动截图；XML 的 if 不是 Java if；六项测试要自己写，不能复制旧脚本。

## 视频、文档与若依对应点

| 资源/文件 | 今天只看、只做这些 |
|---|---|
| [MyBatis 动态 SQL](https://mybatis.org/mybatis-3/dynamic-sql.html) | if/where |
| D10-D15 自己的脚本 | 仅在自测失败后回看 |
...
完成三个时段的操作、一个学习记录和下方勾选项。学习记录保存在项目根目录 docs\learning\day-16\（目录不存在时自行新建），至少放：
...
1. 新建 day16_sql_exam.sql，关闭旧笔记。
2. 完成 DDL、DML、条件查询、连接分组、事务回滚、索引说明六项。
3. 每项末尾标会/需查资料，至少 5 项会才继续。

### 15:00–18:00：按步骤完成练习或项目操作

1. 画 category、record、application 三表 ER，标 id/category_id/archive_id 和 1:N。
2. 建 data_dictionary_draft.md，逐字段填写类型、空值、约束、业务意义。
3. 检查 ER、字典、数据库 SHOW CREATE TABLE 三者一致。

### 19:00–23:00：验证、调试、复盘与安全使用 AI

1. 打开 SysDictTypeMapper.xml 的 selectDictTypeList，列出每个 if 的参数和拼接条件。
2. 在若依库执行等价只读 SELECT，并与页面筛选 rows/total 对比。
3. 整理六项薄弱点与补练计划。
...
能从空白写出一个动态查询的普通 SQL，并说明参数为空时的变化。
...
4. 向 AI 或我提问时使用：今天是 Day 16，我在【步骤号】。现象是【完整提示】；预期是【结果】；我已检查【文件/SQL/Network】。请先给 3 个原因和逐项验证方法，不要重写模块。

---

## 任务参考答案（完成后再查看）

WHERE 处理原始行，GROUP BY 后用 HAVING；XML `<where>` 帮动态条件生成合法 WHERE 并去掉前导 AND。

**完成后复述：** 设计图、字段字典、实际 SQL 必须一致，否则生成器会把错误放大。
