# Day 20：MyBatis Mapper、XML、动态 SQL与结果映射

> **今日唯一目标：** 追懂 Mapper 接口、XML namespace/id、resultMap 与动态条件。
>
> **今天不做：** 不修改 XML，不使用 ${} 拼用户输入。 只完成本日主题；验收未通过就停在当天补练，不用“先往后做”掩盖缺口。

## 知识点速记（完成当天任务后用于复习）

- **Mapper：**接口声明 Java 方法，MyBatis 运行时生成代理。
- **namespace/id：**接口全名与方法名定位 XML SQL。
- **resultMap：**查询列到对象属性映射。
- **if/where：**参数决定条件片段。

### 本日必须形成的业务映射

Service 调 Mapper→Mapper XML→sys_dict_type→List<SysDictType>，页面只接收包装结果。

### 本日重点、易混点与易面试点

接口没有实现类是框架代理；#{} 是绑定参数；结果错不等于先改页面。

## 视频、文档与若依对应点

| 资源/文件 | 今天只看、只做这些 |
|---|---|
| [MyBatis XML](https://mybatis.org/mybatis-3/sqlmap-xml.html) | select/resultMap |
| [MyBatis dynamic SQL](https://mybatis.org/mybatis-3/dynamic-sql.html) | if/where |
| SysDictTypeMapper.java/XML | selectDictTypeList |
...
完成三个时段的操作、一个学习记录和下方勾选项。学习记录保存在项目根目录 docs\learning\day-20\（目录不存在时自行新建），至少放：
...
1. 并排打开 Mapper 与 XML，核对 namespace 和 select id。
2. 定位 resultMap、selectDictTypeVo、selectDictTypeList；各写一句职责。
3. 逐个抄出 if 参数为空/非空的结果。

### 15:00–18:00：按步骤完成练习或项目操作

1. 页面只输入 dictName 查询，记录 Network 参数与后端 Preparing/Parameters 日志。
2. DBeaver 在若依库执行等价 SELECT，只读比较结果。
3. 再加 status 条件，比较 SQL 是否多出 AND。

### 19:00–23:00：验证、调试、复盘与安全使用 AI

1. Mapper 声明、Service 调用、Controller list 三处打断点，记录顺序。
2. 写“参数→if→SQL→resultMap→List”链图。
3. 给自己出题：条件不生效按哪三处查。
...
4. 向 AI 或我提问时使用：今天是 Day 20，我在【步骤号】。现象是【完整提示】；预期是【结果】；我已检查【文件/SQL/Network】。请先给 3 个原因和逐项验证方法，不要重写模块。

---

## 任务参考答案（完成后再查看）

先 Network 参数，再 Controller 对象，再 XML if，再 SQL 日志 Parameters，最后 DBeaver 数据；#{} 不是字符串拼接。

**完成后复述：** MyBatis 用约定连接接口与 XML，动态 SQL由参数决定。
