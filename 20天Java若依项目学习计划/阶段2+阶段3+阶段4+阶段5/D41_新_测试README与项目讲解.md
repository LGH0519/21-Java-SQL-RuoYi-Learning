...
> **今日唯一目标：** 完成 15+ 测试（5+失败）、可复现 README 和三分钟讲解。
>
> **今天不做：** 不执行未经验证的 AI 建议，不删除目录、不重置 Git、不提交秘密。 只完成本日主题；验收未通过就停在当天补练，不用“先往后做”掩盖缺口。

## 知识点速记（完成当天任务后用于复习）

- **核心对象：**今天围绕 docs/test、README.md、ER 图、截图 建立输入、处理、输出三段理解。
...
| 资源/文件 | 今天只看、只做这些 |
|---|---|
| 指定目标：docs/test、README.md、ER 图、截图 | 先逐个搜索、只读指定入口 |
...
完成三个时段的操作、一个学习记录和下方勾选项。学习记录保存在项目根目录 docs\learning\day-41\（目录不存在时自行新建），至少放：
...
2. 在 IDEA/DBeaver/浏览器中定位 docs/test、README.md、ER 图、截图，记录真实文件路径、方法名、表名或菜单路径。
...
- [ ] 已定位并记录：docs/test、README.md、ER 图、截图
...
4. 向 AI 或我提问时使用：今天是 Day 41，我在【步骤号】。现象是【完整提示】；预期是【结果】；我已检查【文件/SQL/Network】。请先给 3 个原因和逐项验证方法，不要重写模块。
...
**完成后复述：** 测试、README 与项目讲解 的关键是先确定边界和证据，再由 AI 或生成器完成最小实现；人工负责规则、审查和验收。
