# Day 19：Spring Boot 与 Spring MVC

> **今日唯一目标：** 只结合字典类型模块理解 URL、参数绑定、Controller 和 JSON 响应。
>
> **今天不做：** 不改 application.yml 或安全配置。 只完成本日主题；验收未通过就停在当天补练，不用“先往后做”掩盖缺口。

## 知识点速记（完成当天任务后用于复习）

- **Boot：**按配置启动应用。
- **MVC：**将 HTTP 请求映射到 Controller 方法。
- **注解：**RestController 返回 JSON，RequestMapping 定义路径，Get/PostMapping 定义方法。
- **绑定：**查询参数/请求体可进入 Java 对象。

### 本日必须形成的业务映射

浏览器 GET 字典列表→Controller list→Service→Mapper；TableDataInfo 最终成为 JSON rows/total。

### 本日重点、易混点与易面试点

8080 是后端接口不是管理页面；200 不必然业务成功；类路径和方法路径拼成 URL。

## 视频、文档与若依对应点

| 资源/文件 | 今天只看、只做这些 |
|---|---|
| [Spring REST 入门](https://spring.io/guides/gs/rest-service/) | Controller/JSON |
| RuoYiApplication.java | main 与启动注解 |
| SysDictTypeController.java | list/add/edit/remove |
...
完成三个时段的操作、一个学习记录和下方勾选项。学习记录保存在项目根目录 docs\learning\day-19\（目录不存在时自行新建），至少放：
...
1. IDEA 搜索 RuoYiApplication，记录 main 与启动端口配置。
2. 只读 application.yml 的 server、datasource、redis 配置名和值。
3. 浏览器打开 8080，记录后端欢迎提示的含义。

### 15:00–18:00：按步骤完成练习或项目操作

1. 标注字典 Controller 的 RestController、RequestMapping、GetMapping、RequestBody、PathVariable。
2. F12 Network 点击查询，记录 URL、Method、Query、Status、rows/total。
3. 将 URL 分拆为代理前缀、类路径、方法路径。

### 19:00–23:00：验证、调试、复盘与安全使用 AI

1. 从 Network 导入一条已登录请求到 Apifox，绝不保存 Token。
2. list 方法打断点，改变一个筛选条件观察 Java 参数对象。
3. 写“URL→参数→Service→JSON”四行说明。
...
能说清 Controller 只负责 HTTP 边界而不是写复杂 SQL。
...
4. 向 AI 或我提问时使用：今天是 Day 19，我在【步骤号】。现象是【完整提示】；预期是【结果】；我已检查【文件/SQL/Network】。请先给 3 个原因和逐项验证方法，不要重写模块。

---

## 任务参考答案（完成后再查看）

GET 查询参数通常绑定对象字段；Post JSON 才常用 RequestBody。具体以 Controller 注解和请求为证据。

**完成后复述：** Controller 是接口门牌与适配层，业务规则在 Service，SQL 在 Mapper/XML。
